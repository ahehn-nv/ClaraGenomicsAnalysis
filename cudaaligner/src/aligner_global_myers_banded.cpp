...
StatusType AlignerGlobalMyersBanded::align_all()
{
    ...
    {
        seq_starts_d.clear();
        seq_starts_d.resize(2 * n_alignments + 1);
    }
    if (get_size(result_starts_d) < n_alignments + 1)
    {
        result_starts_d.clear();
        result_starts_d.resize(n_alignments + 1);
    }
    if (get_size(result_lengths_d) < n_alignments)
    {
        result_lengths_d.clear();
        result_lengths_d.resize(n_alignments);
    }
